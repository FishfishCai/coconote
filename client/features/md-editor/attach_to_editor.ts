// Wire a Yjs collab session into the editor for the file `id`. Owns the
...
/**
 * Attach a collab session for the file `id`. The caller has already
 * setState'd an empty doc so the first SYNC_STEP_2 lands cleanly. If
 * collab yields no text within 3s, `fallbackText` is dispatched in and the
 * handle torn down so subsequent edits save via the regular PUT path.
 */
export function attachCollab(client: Client, id: string, fallbackText: string): void {
  const fallbackTimer = setTimeout(() => {
    if (client.currentId() !== id) return;
  ...
  import("../../capabilities/collab/index.ts").then(({ connectCollab }) => {
    if (client.currentId() !== id) {
