...
// markdown.md: only H1-H4 exist. H5/H6 (`##### x`) are not headings, so
// leave their marks visible and they render literally - matching the
// static renderer (markdown_render.ts).
const ATX_HEADING_1_4 = /^ATXHeading[1-4]$/;
  ...
  return decoratorStateField((state: EditorState) => {
    ...
    tree.iterate({
      enter({ name, from, to, node }) {
        if (name === "HeaderMark" && ATX_HEADING_1_4.test(node.parent?.name ?? "")) {
