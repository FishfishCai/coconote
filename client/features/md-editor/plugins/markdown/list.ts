...
// list parser (parser.ts) is constrained to match, so `+`/`*`/`1)` are
// plain text and never reach this decorator as ListMark nodes.
const BULLET_RE = /^-/;
const ORDERED_RE = /^(\d+)\./;
    ...
    syntaxTree(state).iterate({
      enter({ name, from, to, node }) {
        ...
        if (BULLET_RE.test(mark)) {
          // Unordered markers cycle a glyph by nesting depth (markdown.md),
          // same depth source as the ordered branch below.
          const depth = orderedDepth(node);
          widgets.push(
            Decoration.replace({ widget: new BulletWidget(depth, to) })
              .range(from, to),
          );
          return;
        }
        ...
        if (om) {
          ...
          const depth = orderedDepth(node);
          // Replace only the digits - keep the trailing `.` so source
          // width is preserved.
...
// Unordered bullets cycle by nesting depth, repeating every four levels
// (markdown.md). `depth` is the 1-based list depth from orderedDepth, so
// level 1 maps to the first glyph, level 5 wraps back to it.
const BULLET_GLYPHS = ["•", "◦", "▪", "‣"];

function bulletGlyph(depth: number): string {
  return BULLET_GLYPHS[(depth - 1) % BULLET_GLYPHS.length];
}

class BulletWidget extends WidgetType {
  constructor(readonly depth: number, readonly caretPos: number) {
    super();
  }
  ...
  toDOM(view: EditorView): HTMLElement {
    ...
    span.textContent = bulletGlyph(this.depth);
