...
type Client = EditorCtx & ConfigCtx & UICtx;
...
class TableWidget extends CachedWidget<Client> {
  ...
  toDOM(): HTMLElement {
    ...
    el.innerHTML = renderMarkdownToHtml(parseMarkdown(this.source), {
      // SPEC: wikilink chips show the target title; the table lives in the
      // current buffer so local images resolve against the current file.
      wikiLinkTitle: buildWikiLinkTitle(this.ctx),
      assetOwnerId: this.ctx.currentId(),
    });
