...
import { assetUrl, authedFetch } from "../../../core/transport";
 ...
 *  an extension derived from the MIME type, PUTs it into the page's
 *  `.<name>.assets/` via `/.file?id=<owner>&asset=<name>`, then inserts
...
export async function handleImagePaste(
  ...
  if (!ownerId || client.currentPageMeta()?.kind !== "md") return;
  const ext = pickExt(file.type, file.name);
  // Date-based stems collide when two pastes land in the same
  // millisecond or the same file is dropped twice - append random chars
  // for uniqueness inside the assets dir.
  const rnd = Math.floor(Math.random() * 0x7fffffff).toString(36);
  const name = `pasted-${Date.now().toString(36)}-${rnd}${ext}`;
  try {
    const bytes = new Uint8Array(await file.arrayBuffer());
    const r = await authedFetch(assetUrl(ownerId, name), {
      method: "PUT",
