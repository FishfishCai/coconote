...
import { refsMaintenancePlugin } from "./plugins/links/refs_maintenance.ts";
...
const BASE_EXTENSIONS: Array<(c: Client) => Extension[]> = [
  ...
  (c) => [snippetsPlugin(c)],
  // Keeps refs/backrefs in sync with the buffer's links (self-guards
  // read-only / non-md, so it is harmless in read mode).
  (c) => [refsMaintenancePlugin(c)],
];
