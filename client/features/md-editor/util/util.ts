...
/** Lezer-markdown node names whose text content is literal code: the
 *  math scanner / autocomplete / image scanner must NOT look inside
 *  these. Use against `node.type.name`. markdown.md has no comment
 *  construct, so `<!-- -->` is treated as ordinary text (not listed). */
