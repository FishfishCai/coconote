...
export function createEditorState(
  ...
  const collabAlive = client.collabHandle?.id === pageId;
  ...
  // handle bound to THIS page id (rebuildEditorState scenario), reseed
  ...
  const liveCollab = client.collabHandle?.id === pageId
  ...
  const readOnlyExtensions = client.editModeCompartment.of(
    editModeExtensionsFor(readOnly, editorMode),
  );
...
export function editModeExtensionsFor(
  ...
  // Edit availability is per-file (X-Permission -> pageReadOnly) plus the
  // read editor mode; there is no global read-only flag.
  const isRO = pageReadOnly || editorMode === "read";
  return [
