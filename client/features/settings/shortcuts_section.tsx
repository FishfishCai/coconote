...
export function ShortcutsSection({ client }: { client: Client }) {
  ...
  useEffect(() => {
    ...
    const onKey = (ev: KeyboardEvent) => {
      ...
      // Cmd/Ctrl +/-/0 are reserved for per-reader zoom (keyboard.ts), so
      // refuse to bind them here - otherwise the binding would be silently
      // dead (zoom always wins). Keep recording so the user can pick
      // another combo.
      if (isReservedZoomCombo(ev)) return;
  ...
  return (
    <section>
      ...
      <p className="coconote-settings-hint">
        ...
        Backspace), the reader zoom keys (Mod +/-/0), and system
        shortcuts use the defaults and cannot be rebound.
      </p>
