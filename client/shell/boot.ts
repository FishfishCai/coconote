...
import { authedFetch, setAuthToken } from "../core/transport";
import { Client } from "./client.ts";
import { Config } from "../core/config.ts";
import { wireOsFileOpen } from "./os_open.ts";
import { readUserPrefs, USER_PREFS_KEY } from "../core/config/index.ts";
...
safeRun(async () => {
  ...
  try {
    // The fetch surfaces the auth gate (notAuthenticatedError) and warms
    // the offline cache. The body itself is no longer read at boot - edit
    // availability is per-file (X-Permission header), not a global flag.
    await cachedFetch(".config");
  } catch (e: unknown) {
  ...
  // UI prefs (appearance, shortcuts, snippets) live entirely in
  // localStorage (the server ships no `ui` block, and the snippet.json
  // sidecar was dropped) - Settings edits persist without touching the yaml.
  ...
  // setting.md L96: open files double-clicked in the OS file manager or
  // dragged into the window (desktop shell only - a browser sees neither).
  wireOsFileOpen(client);

  // setting.md L104: settings are shared across windows/tabs. When another
  // window rewrites the prefs (or the dark-mode first-paint key), re-apply
  // them here live - no reload. The `storage` event only fires in OTHER
  // windows, so this never reacts to its own writes.
  globalThis.addEventListener("storage", (e: StorageEvent) => {
    if (e.key === USER_PREFS_KEY || e.key === "coconote.darkMode") {
      client.syncPrefsFromStorage();
    }
  });
});
