...
export type AppViewState = {
  ...
  /** One-shot filter consumed by the Content browser on next mount. */
  pendingContentFilter: string;
  /** PDF viewer state when active (path + optional anchor); null when
...
export type BootConfig = {
  readOnly: boolean;
  /** Raw text of snippet.json on disk (editor.md §Snippet) — same
   *  lookup path as coconote.yaml. Empty when the file isn't present. */
  snippets?: string;
};
