...
import { getAuthToken } from "../../core/transport";
...
/**
 * Connect to a collab session for the file `id`. Attach `extension` to
 * your EditorState, call `disconnect()` on unmount.
 */
export function connectCollab(id: string): CollabHandle {
  const doc = new Y.Doc();
  ...
  const wsUrl = (() => {
    ...
    // spec server.md: the room is keyed by the file id, passed as `?id=`
    // (the router matches `/.collab` exactly). Not a path segment.
    const base = `${proto}://${location.host}/.collab?id=${
  ...
  // Window/tab close tears down the renderer without ever calling
  // disconnect(), so peers in OTHER windows viewing the same page would
  // keep this cursor until their local 30s awareness timeout. pagehide
  // (fires on real unload AND bfcache) lets us send the removal frame
  // first. Harmless if disconnect() also runs later: the socket is then
  // shut, so sendAwarenessRemove() no-ops.
  const onPageHide = () => sendAwarenessRemove();
