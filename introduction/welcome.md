...
The file lives in the standard per-user config directory — `~/.config/coconote/` on macOS / Linux (honouring `$XDG_CONFIG_HOME`), `%APPDATA%\coconote\` on Windows — which the server checks for a parseable `coconote.yaml` on every boot. The desktop app's Setting can redirect it elsewhere.
...
- **Remote root** — one URL under `url:`, pointing at another coconote server. Connecting mounts **all** of that server's roots (its own `root:` mapping) into your vault at once; you don't choose their names — they come from the remote yaml.
