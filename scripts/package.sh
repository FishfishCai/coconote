...
# Flags: --server-only (server zips only), --app-only (installers only),
...
for arg in "$@"; do
  case "$arg" in
    # An --X-only flag narrows the build to just X.
...
build_server() {
  ensure_client_bundle
  # Target list comes from scripts/targets.sh (the canonical source shared with
  # the Makefile and .github/workflows/release.yml). Locally every triple is
  # cross-built with `cargo zigbuild`; the runner/use_zigbuild columns are
  # CI-only and ignored here.
  source "$ROOT/scripts/targets.sh"
  for row in "${COCONOTE_TARGETS[@]}"; do
    read -r triple os arch bin runner use_zigbuild <<<"$row"
    ...
    # standard config dir on first launch.
