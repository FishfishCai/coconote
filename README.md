...
- **Wikilinks and graph view.** Cross-link documents with `[[...]]`, follow hover previews, and see a file's neighborhood in a current-file graph view.
...
- **Self-hosted, files you own.** Run the native desktop app on macOS, Windows, or Linux, or run a headless server and reach your notes from any browser on your network. Your notes stay as plain markdown and PDF files on disk.
...
The server is a single static-friendly binary that serves both the HTTP API and the built-in web client. Unpack and run it, then open the URL it logs in any browser. First launch auto-creates `coconote.yaml` in the standard per-user config dir. Open files from the app (the open dialog or the recent list); edit `coconote.yaml` for the port, the auth token, and any remote instances.
...
The desktop app bundles a sidecar server, so no separate server install is needed. First launch writes a default `coconote.yaml` under the standard per-user config dir (`~/.config/coconote/` on macOS/Linux, `%APPDATA%\coconote\` on Windows). The generated file is commented, so you can edit the port, the auth token, and any remote instances in place.
