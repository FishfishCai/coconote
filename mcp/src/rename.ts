...
// links that resolved to the renamed page before AND no longer do after
...
export async function refactorLinks(oldFullPath: string, newFullPath: string): Promise<number> {
  ...
  const before = after.map((p) => (p.name === newName ? { ...p, name: oldName } : p));
  ...
  for (let i = 0; i < mdPaths.length; i += BATCH) {
    const results = await Promise.all(
      mdPaths.slice(i, i + BATCH).map(async (p) => {
        ...
        if (!got) return false;
        const rewritten = rewriteOne(got.text, oldName, newName, before, after);
 ...
 *  pointed at the old name. Companion and wikilink follow-ups are
 *  best-effort, like the client. */
