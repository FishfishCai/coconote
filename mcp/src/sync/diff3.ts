// Line-based 3-way merge (history.md Three-way diff), ported verbatim
// from client/lib/diff3.ts. Region-based diff3: a "stable" line is a
// base line kept verbatim by BOTH sides, each region between stable
// anchors is classified whole (only local changed -> local, only remote
// -> remote, identical change -> either, different -> conflict).
// Deletion counts as a change, so modify-vs-delete conflicts instead of
// taking the surviving side.
...
export function merge3(base: string, local: string, remote: string): Chunk[] {
  ...
  while (b < baseLines.length) {
    ...
    const ls = ml.get(s)!;
    const rs = mr.get(s)!;
    emitRegion(baseLines.slice(b, s), localLines.slice(l, ls), remoteLines.slice(r, rs));
  ...
  const nl = localNl === remoteNl ? localNl : localNl === baseNl ? remoteNl : localNl;
  if (!nl && out.length > 0) {
