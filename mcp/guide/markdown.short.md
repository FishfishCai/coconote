...
- No tables, no HTML tags, no footnotes. They are not part of the dialect.
- Lists: `-` unordered, `1.` ordered. Sub-items are indented by exactly 4 spaces.
  Ordered numbering is positional, the typed digits are ignored.
...
- Quote: `>` per level, stack `> >` to nest. Horizontal rule: `---`, `***`, or `___` on its own line.
...
- Callout: `::: kind` (optional label: `::: kind:label`), body lines, closed by `:::`.
